package com.android.design.drawable;
...
public class RippleDrawable extends Drawable implements Animatable,	OnTouchListener {
	...
	protected static final String TAG = "RippleDrawable";
	...
	public static final int FLAT_BUTTON = 0;
	public static final int PAPER_BUTTON = 1;
	...
	private RippleDrawable(Drawable backgroundDrawable, int backgroundAnimDuration, int backgroundColor, int rippleType, int delayClickType,  int maxRippleRadius, int rippleAnimDuration, int rippleColor, Interpolator inInterpolator, Interpolator outInterpolator, int type, int topLeftCornerRadius, int topRightCornerRadius, int bottomRightCornerRadius, int bottomLeftCornerRadius, int left, int top, int right, int bottom){
		...
		if(mRippleType == TYPE_WAVE)
			mOutShader = new RadialGradient(0, 0, GRADIENT_RADIUS, new int[]{0, Util.getColor(mRippleColor, 0f), mRippleColor}, GRADIENT_STOPS, Shader.TileMode.CLAMP);		
	}
	...
	private void updateTouch(){		
		if(mState != STATE_RELEASE){
			final float backgroundProgress = Math.min(1f, (float)(SystemClock.uptimeMillis() - mStartTime) / mBackgroundAnimDuration);
			mBackgroundAlphaPercent = mInInterpolator.getInterpolation(backgroundProgress) * Color.alpha(mBackgroundColor) / 255f;
			final float touchProgress = Math.min(1f, (float)(SystemClock.uptimeMillis() - mStartTime) / mRippleAnimDuration);			
	...
	private void updateWave(){
		final float progress = Math.min(1f, (float)(SystemClock.uptimeMillis() - mStartTime) / mRippleAnimDuration);
		if(mState != STATE_RELEASE){			
	...
	public static class Builder{
		...
		private float bgAlpha = 1.5f;
		private float rippleAlpha = 1.5f;
		
		public Builder(){}

        public Builder(final Context context, final View view, final int defStyleRes){
            this(context, view, null, 0, defStyleRes);
        }

		public Builder(final Context context, final View view, final AttributeSet attrs, final int defStyleAttr, final int defStyleRes){
			this.mView = view;
			int type, resId;
			final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.RippleDrawable, defStyleAttr, defStyleRes);
			final int buttonType = a.getInteger(R.styleable.RippleDrawable_rd_buttonType, -1);
			float alpha = a.getFloat(R.styleable.RippleDrawable_rd_alpha, 0.1f);
			bgAlpha = alpha;
            rippleAlpha = alpha;
			bgColor = a.getColor(R.styleable.RippleDrawable_rd_backgroundColor, Color.parseColor("#33999999"));
			rippleColor = a.getColor(R.styleable.RippleDrawable_rd_rippleColor, Util.colorControlHighlight(context, Color.parseColor("#33999999")));
			
			// Create Button if view is button type.
			buttonBuilder(attrs, buttonType);
			
            // Set background color
            backgroundColor(Util.getColorWithAlpha(bgColor, bgAlpha));
            ...
            if(type >= TypedValue.TYPE_FIRST_INT && type <= TypedValue.TYPE_LAST_INT){
			    ...
			    maxRippleRadius(a.getDimensionPixelSize(R.styleable.RippleDrawable_rd_maxRippleRadius, Util.convertDpToPx(context, 48)));
            }
            
			// set ripple color
            rippleColor(Util.getColorWithAlpha(Util.getDarkColor(rippleColor), rippleAlpha));
		...
		public void buttonBuilder(final AttributeSet attrs, final int buttonType){
			// Get background from android xml tag (android:background).
            final int backgroundColor = attrs.getAttributeResourceValue(DesignConstants.ANDROIDXML, "background", -1);
            // Check background is not -1 (value = "@null").
            if(backgroundColor != -1 && buttonType == RippleDrawable.PAPER_BUTTON){
            	final Drawable background = mView.getBackground();
            	// Get color from background view.
            	if (background instanceof ColorDrawable){
            		color = ((ColorDrawable) background).getColor();
            		// Set background color for ripple color and select background color.
            		bgColor = color;
            		rippleColor = color;
            		
            		// Set Alpha color for background color and ripple color on Touch.
            		bgAlpha = 0.4f;
            		rippleAlpha = 0.9f;
            	}else{
            		color = Color.parseColor("#33999999");
            	}
            	// Get layer drawable from xml.
            	mBackgroundDrawable = mView.getContext().getResources().getDrawable(R.drawable.button_background);
            	// Cast LayerDrawable from drawable background.
            	final LayerDrawable layerShape = (LayerDrawable) mBackgroundDrawable;
            	if(null != mBackgroundDrawable){
            		// Cast Gradient drawable from layer drawable to set new color.
            		final GradientDrawable shape = (GradientDrawable) layerShape.findDrawableByLayerId(R.id.shape_bacground);
            		if(null != shape){
                		shape.setColor(color);
                	}
            	}
            }else if(buttonType == RippleDrawable.FLAT_BUTTON){
            	if(backgroundColor != -1){
            		...
            		// Get layer drawable from xml.
                	mBackgroundDrawable = mView.getContext().getResources().getDrawable(android.R.color.transparent);
            	}
            }
		}
