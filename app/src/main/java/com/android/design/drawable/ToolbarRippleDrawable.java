...
public class ToolbarRippleDrawable extends Drawable implements Animatable {
	...
	protected boolean onStateChange(int[] state) {		
		final boolean pressed = Util.hasState(state, android.R.attr.state_pressed);
		if(mPressed != pressed){
			mPressed = pressed;
			if(mPressed){
	...
	private int getMaxRippleRadius(float x, float y){
		final float x1 = x < mBackgroundBounds.centerX() ? mBackgroundBounds.right : mBackgroundBounds.left;
		final float y1 = y < mBackgroundBounds.centerY() ? mBackgroundBounds.bottom : mBackgroundBounds.top;
	...
	public static class Builder{
		...
		public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
            ...
            int type = Util.getType(a, R.styleable.RippleDrawable_rd_maxRippleRadius);
