...
public class CheckBoxDrawable extends Drawable implements Animatable {
	...
	private void drawChecked(Canvas canvas){
		...
		if(isRunning()){
			if(mAnimProgress < FILL_TIME){
				...
				mPaint.setColor(Util.getMiddleColor(mPrevColor, mCurColor, progress));
	...
	protected boolean onStateChange(int[] state) {
		boolean checked = Util.hasState(state, android.R.attr.state_checked);		
	...
	public static class Builder{
		...
		public Builder(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
			final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CheckBoxDrawable, defStyleAttr, defStyleRes);
			width(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_checkbox_width, Util.convertDpToPx(context, 32)));
			height(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_checkbox_height, Util.convertDpToPx(context, 32)));
			boxSize(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_checkbox_boxSize, Util.convertDpToPx(context, 18)));
			cornerRadius(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_checkbox_cornerRadius, Util.convertDpToPx(context, 2)));
			strokeSize(a.getDimensionPixelSize(R.styleable.CheckBoxDrawable_checkbox_strokeSize, Util.convertDpToPx(context, 2)));
			strokeColor(a.getColorStateList(R.styleable.CheckBoxDrawable_checkbox_strokeColor));
			tickColor(a.getColor(R.styleable.CheckBoxDrawable_checkbox_tickColor, 0xFFFFFFFF));
			animDuration(a.getInt(R.styleable.CheckBoxDrawable_checkbox_animDuration, context.getResources().getInteger(android.R.integer.config_mediumAnimTime)));
			a.recycle();
			...
			if(mStrokeColor == null){
				...
				final int[] colors = new int[]{
						Util.colorControlNormal(context, 0xFF000000),
						Util.colorControlActivated(context, 0xFF000000),
				};				
